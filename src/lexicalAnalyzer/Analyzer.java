...
import util.SpecificWords;

import java.util.ArrayList;

public class Analyzer {
    public ArrayList<Token> analyze(char[] chars) {
        // 结果的token序列
        ...
        while (chars[index] != '\0') {
            ...
            if (isLetter(chars[index])) {
                ...
                while (isLetter(chars[index + 1]) || isDigit(chars[index + 1])) {
                    index++;
                    temp += chars[index];
                }
...
//                    System.out.println("整数常数:(" + 100 + "," + temp + ")");
            ...
            //既不是数字也不是字母也不是空格，则为界符或运算符或注释，跳过空格
            else if (chars[index] != ' ') {
                temp += chars[index];
...
//                    System.out.println("界符:(" + isDivide(temp) + "," + temp + ")");
                    temp = "";
                    index++;
                    continue;
                }
                //判断是否为运算符
                    ...
                    //若下一个字符也是符号类型则加入temp
                    while ((isDivide(chars[index + 1] + "") == -1) && (!isDigit(chars[index + 1]))
                            && (!isLetter(chars[index + 1]))) {
                        index++;
    ...
    private char[] removeComment(char[] chars) {
    ...
    //判断是否为保留字,并返回编号
    private int isReserve(String s) {
